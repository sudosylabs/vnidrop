...
def assemble(input_dir, output_dir, version, number, commit, platforms="windows,linux,android,macos"):
    ...
    if not selected or len(set(selected)) != len(selected) or set(selected) - {"windows", "linux", "android", "macos"}:
        raise ValueError("Select valid preview platforms")
    ...
    manifest = {
        ...
        **({"appleDirectBuildNumber": apple["directBuildNumber"]} if apple else {}),
        **({"windowsInstallerSigned": False} if "windows" in selected else {}),
        "files": files,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--platforms", default="windows,linux,android,macos")
    parser.add_argument("--input", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    args = parser.parse_args()
    try:
        assemble(args.input, args.output, args.version, args.number, args.commit, args.platforms)
