...
| `linux` | `linux/data/strings.json` | Embedded GNOME catalog, named arguments and plural categories preserved |
...
**Never edit the generated `.xcstrings` / `strings.xml` / `.resw` / Linux catalog by hand** — edit the source `localization/strings.json` and
...
- **`targets`** *(optional)* — any of `kmp`, `apple`, `windows`, and `linux`. Omit to mean **all** targets.
