...
Set `VNIDROP_APPLE_INTEL=1` to also build `x86_64-apple-darwin`. The
direct-download script sets that. `xcodebuild -create-xcframework` only accepts
one library per platform, so those two macOS archives are combined first. Each
app is still built with a single `ARCHS` value, so each download stays one
architecture.
